...
app.get('/displaymaterial', function(request, response) {
        connection.query('select * from clases', function (error, results, fields) {
          if (error) throw error;
          document.getElementById("seleccion").innerHTML = `<select id="clases" name="select" onchange="mostrarmaterial()" default='1'>
          ${results.map((e)=>{
            return `<option value=${e.results.nombre_clase}>${e.results}</option>`
          })}
            </select>`
        });
});
